    ...
    <section id="about" className="py-12 bg-white space-y-[100px]">
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-2">
          {/* {team.map(member => (
            <div key={member.id}>
              <img src={member.image} alt={member.name} className="rounded shadow-lg w-full" />
          ...
          {team.map((member, index) => (
            ...
            >
              ...
              <div className="sm:w-1/2 flex flex-col justify-center m-3">
                <h1 className={`text-2xl font-bold ${index % 2 === 0 ? 'sm:text-right' : 'sm:text-left'} sm:items-center gap-6 text-center mb-5`}>{member.name}</h1>
